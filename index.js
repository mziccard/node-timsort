...
function sort(array, compare, lo, hi) {
  ...
  if (remaining < DEFAULT_MIN_MERGE) {
    ...
    binarySort(array, lo, hi, lo + runLength, compare);
    return;
  }
  ...
  do {
    ...
    if (runLength < minRun) {
      ...
      binarySort(array, lo, lo + force, lo + runLength, compare);
      runLength = force;
    }
 ...
 * @param {function} compare - Item comparison function.
 */
function binarySort(array, lo, hi, start, compare) {
  if (start === lo) {
    start++;
  }
  for (; start < hi; start++) {
...
/**
 * A TimSort object is used to sort an array with TimSort.
 *
 * @param {array} array - The array to sort.
 * @param {function} compare - Item comparison function.
 */
...
TimSort.prototype.mergeLow = function(start1, length1, start2, length2) {
  ...
  while (true) {
    ...
    do {
      count1 =
        gallopRight(array[cursor2], tmp, cursor1, length1, 0, compare);
      if (count1 !== 0) {
        for (i = 0; i < count1; i++) {
          array[dest + i] = tmp[cursor1 + i];
        }
      ...
      count2 =
        gallopLeft(tmp[cursor1], array, cursor2, length2, 0, compare);
      if (count2 !== 0) {
        for (i = 0; i < count2; i++) {
          array[dest + i] = array[cursor2 + i];
        }
